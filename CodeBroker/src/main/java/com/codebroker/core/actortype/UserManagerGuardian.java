...
public class UserManagerGuardian {
	...
	private ISession.SessionAcceptMessage message;

	public static Behavior<IUserManager> create(ActorRef<ISession> self, ISession.SessionAcceptMessage message) {
		return Behaviors.setup(context -> new UserManagerGuardian(context,self,message).handle());
	}

	private UserManagerGuardian(ActorContext<IUserManager> context, ActorRef<ISession> self,ISession.SessionAcceptMessage message) {
		this.self=self;
		...
		context.getSystem().receptionist().tell(Receptionist.subscribe(ServiceKey.create(IUserManager.class, "UserManager."+intProperty), listingAdapter));
	}
	...
	private Behavior<IUserManager> handle() {
		return Behaviors.receive(IUserManager.class)
				.onMessage(IUserManager.AddProcessorReference.class, listing -> {
					PropertiesWrapper propertiesWrapper = ContextResolver.getPropertiesWrapper();
					int intProperty = propertiesWrapper.getIntProperty(SystemEnvironment.APP_ID, 1);
					serviceInstances = listing.listing.getServiceInstances(ServiceKey.create(IUserManager.class, "UserManager."+intProperty));
					for (ActorRef<IUserManager> serviceInstance : serviceInstances) {
						serviceInstance.tell(new IUserManager.TryBindingUser(self,message.message));
					}
